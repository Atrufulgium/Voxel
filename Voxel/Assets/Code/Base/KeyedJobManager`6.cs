...
namespace Atrufulgium.Voxel.Base {
    ...
    public abstract class KeyedJobManager<K, J1, J2, J3, TInput, TResult> : IDisposable
        ...
        static readonly Stack<KeyedJobManager<K, J1, J2, J3, TInput, TResult>> idleJobbers = new();
        static readonly Dictionary<K, KeyedJobManager<K, J1, J2, J3, TInput, TResult>> activeJobbers = new();
        ...
        /// </remarks>
        public static void RunAsynchronously<TManager>(K key, TInput input) where TManager : KeyedJobManager<K, J1, J2, J3, TInput, TResult>, new() {
            if (JobExists(key))
                throw new ArgumentException("The given key already has an asssociated job, so it cannot have a new one.");

            KeyedJobManager<K, J1, J2, J3, TInput, TResult> keyedJob;
        ...
        /// </summary>
        public static void RunSynchronously<TManager>(TInput input, ref TResult result) where TManager : KeyedJobManager<K, J1, J2, J3, TInput, TResult>, new() {
            KeyedJobManager<K, J1, J2, J3, TInput, TResult> keyedJob;
            if (idleJobbers.Count == 0) {
                keyedJob = new TManager();
            } else {
